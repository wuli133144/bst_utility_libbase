...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
    ...
    while (running)
	{
		...
		{
			INFO("select failed, error code: %d", GetLastError());
			Sleep(MIN_TIMER_DURATION);
		...
		{
			//INFO("select return read count=%d\n", read_set.fd_count);
		...
		{
			//INFO("select return write count=%d\n", write_set.fd_count);
		...
		{
			//INFO("select return exception count=%d\n", excep_set.fd_count);
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
    ...
    while (running)
	{
		...
		for (int i = 0; i < nfds; i++)
		{
			...
			{
				//INFO("OnRead, socket=%d\n", ev_fd);
				pSocket->OnRead();
			}

			if (events[i].filter == EVFILT_WRITE)
			{
				//INFO("OnWrite, socket=%d\n", ev_fd);
				pSocket->OnWrite();
			}
...
void CEventDispatch::AddEvent(SOCKET fd, uint8_t socket_event)
{
	...
	{
		INFO("epoll_ctl() failed, errno=%d, %s", errno, strerror(errno));
	}
}
...
void CEventDispatch::RemoveEvent(SOCKET fd, uint8_t socket_event)
{
	...
	{
		INFO("epoll_ctl failed, errno=%d", errno);
	}
}
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
	...
	while (running)
	{
		...
		for (int i = 0; i < nfds; i++)
		{
            ...
            {
                //INFO("On Peer Close, socket=%d, ev_fd);
                pSocket->OnClose();
            }
            #endif
            // Commit End
			...
			if (events[i].events & EPOLLIN)
			{
                ...
                if (events[i].events & (EPOLLPRI | EPOLLERR | EPOLLHUP))
                {
                    DEBUG("EPOLLIN  socket=%d, events=%#x", ev_fd, events[i].events);
                }
