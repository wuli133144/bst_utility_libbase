...
CBaseSocket::~CBaseSocket()
{
	DEBUG("CBaseSocket::~CBaseSocket, socket=%d", m_socket);
}
...
int CBaseSocket::Listen(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	if (ret == SOCKET_ERROR)
	{
		INFO("bind failed, err_code=%d", _GetErrorCode());
	...
	if (ret == SOCKET_ERROR)
	{
		INFO("listen failed, err_code=%d", _GetErrorCode());
		closesocket(m_socket);
		return NETLIB_ERROR;
	}
	...
	INFO("CBaseSocket::Listen on %s:%d", server_ip, port);
...
{
	INFO("CBaseSocket::Connect, server_ip=%s, port=%d", server_ip, port);
	...
	if (m_socket == INVALID_SOCKET)
	{
		INFO("socket failed, err_code=%d", _GetErrorCode());
	...
	{	
		INFO("connect failed, err_code=%d", _GetErrorCode());
		closesocket(m_socket);
		return NETLIB_INVALID_HANDLE;
	}
...
#endif
			ret = 0;
			//INFO("socket send block fd=%d", m_socket);
		}
		else
		{
			INFO("!!!send failed, err_code: %d,  %s, socket:%d", err_code, strerror(errno), m_socket);
		}
	}
...
void CBaseSocket::OnRead()
{
	...
	else
	{
		...
		{
            DEBUG("ioctlsocket , err_code=%d, %s, avail:%d, socket:%d", _GetErrorCode(), strerror(errno), avail, m_socket);
...
#endif
		if (error) {
            DEBUG("getsockopt failed, err_code=%d, %s", _GetErrorCode(), strerror(errno));
			m_callback(m_callback_data, NETLIB_MSG_CLOSE, (net_handle_t)m_socket, NULL);
		} else {
...
void CBaseSocket::SetSendBufSize(uint32_t send_size)
{
	...
	if (ret == SOCKET_ERROR) {
		INFO("set SO_SNDBUF failed for fd=%d", m_socket);
	}

	socklen_t len = 4;
	int size = 0;
	getsockopt(m_socket, SOL_SOCKET, SO_SNDBUF, &size, &len);
	INFO("socket=%d send_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::SetRecvBufSize(uint32_t recv_size)
{
	...
	if (ret == SOCKET_ERROR) {
		INFO("set SO_RCVBUF failed for fd=%d", m_socket);
	}

	socklen_t len = 4;
	int size = 0;
	getsockopt(m_socket, SOL_SOCKET, SO_RCVBUF, &size, &len);
	INFO("socket=%d recv_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::_SetNonblock(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		INFO("_SetNonblock failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetReuseAddr(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		INFO("_SetReuseAddr failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetNoDelay(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		INFO("_SetNoDelay failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetAddr(const char* ip, const uint16_t port, sockaddr_in* pAddr)
{
	...
	if (pAddr->sin_addr.s_addr == INADDR_NONE)
	{
		...
		if (host == NULL)
		{
			INFO("gethostbyname failed, ip=%s", ip);
			return;
		}
...
void CBaseSocket::_AcceptNewSocket()
{
	...
	while ( (fd = accept(m_socket, (sockaddr*)&peer_addr, &addr_len)) != INVALID_SOCKET )
	{
		...
		DEBUG("AcceptNewSocket, socket=%d from %s:%d", fd, ip_str, port);
