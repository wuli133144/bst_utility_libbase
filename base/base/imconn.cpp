...
void imconn_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
	...
	//INFO("msg=%d, handle=%d ", msg, handle);
	...
	case NETLIB_MSG_CLOSE:
		...
		break;
	default:
		INFO("!!!imconn_callback error msg: %d ", msg);
		break;
	}
...
CImConn::~CImConn()
{
	//INFO("CImConn::~CImConn, handle=%d ", m_handle);
}
...
//	++g_send_pkt_cnt;
	...
	if (remain > 0)
	{
		...
		m_busy = true;
		INFO("send busy, m_handle=%d, remain=%d ", m_handle, m_out_buf.GetWriteOffset());
	}
    else
    {
        OnWriteCompelete();
...
//			++g_recv_pkt_cnt;
		...
		INFO("!!!catch exception, sid=%u, cid=%u, err_code=%u, err_msg=%s, close the connection ",
...
void CImConn::OnWrite()
{
	...
	DEBUG("onWrite, remain=%d ", m_out_buf.GetWriteOffset());
}
