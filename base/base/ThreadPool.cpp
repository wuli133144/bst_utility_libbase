...
void CWorkerThread::Execute()
{
	...
	while (true) {
		...
		TRACE("Thread (%u) has executed %d tasks, list remains %d tasks.", m_thread_idx, m_task_cnt, m_task_current_cnt);

		if(m_task_current_cnt > LIST_WARN_SIZE)
		{
			WARN("Thread (%u) has executed %d tasks, list remains %d tasks.", m_thread_idx, m_task_cnt, m_task_current_cnt);
		}
	}
}
