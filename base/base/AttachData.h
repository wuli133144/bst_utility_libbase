...
enum {
    ...
    ATTACH_TYPE_NOTIFY_AND_PDU = 6,
    ATTACH_TYPE_MSG_AND_PDU = 7,
    ATTACH_TYPE_OLD_VERSION_AND_PDU = 8,
    ATTACH_TYPE_CONSULT_AND_PDU = 9,
};
...
public:
	...
	uint32_t GetType() { return m_type; }
	uint32_t GetHandle() { return m_handle; }
    uint32_t GetNotifyType() { return m_handle; }
    uint32_t GetMsgType() { return m_handle; }
    uint32_t GetServiceType() { return m_service_type; }
